...
def fetch_weather_history(days: int = 14) -> list[tuple[str, float, float]]:
    """Daily (date, temp_max, temp_min) in °C for the last ``days`` days."""
    past = max(1, min(days, 92)) - 1
    try:
        response = httpx.get(
    ...
    for day, tmax, tmin in zip(
        daily.get("time", []),
        daily.get("temperature_2m_max", []),
        daily.get("temperature_2m_min", []),
    ):
        if tmax is not None and tmin is not None:
            out.append((day, round(float(tmax), 1), round(float(tmin), 1)))
    return out
